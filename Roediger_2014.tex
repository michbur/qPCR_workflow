...
by the software and does not attain publication quality. Users who have no 
...
Again, we used the \code{CPP} function to pre-process the raw data. Similar to 
...
the $1^{th}$ and $190^{th}$ data point.
...
The pre-processing lead to data, ready for the analysis of the $Cq_{t}$ value. 
It is important to not that the trend correction and proper baseline was a 
requirement for a sound calculation. The thershold level We calculated $Cq_{t}$ 
...
MM-estimator). Finally, we used the \code{th.cyc} function from the 
\CRANpkg{chipPCR} package to calculate the cycle threshold time for samples D1 
...
Simulations are part in many educational curricula and support teaching 
