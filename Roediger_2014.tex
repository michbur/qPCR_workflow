...
\author{by Stefan R\"{o}diger, Micha\l{} Burdukiewicz and Konstantin 
...
Several software solutions have been developed, which are either distributed as 
...
excellent environment for a reproducible and transparent analysis of data. 
However, for newcomers it is often very challenging to master R from reading the 
manuals and FAQs the not so obvious steps. Here we describe exemplary work-flows 
for the analysis of dPCR or qPCR experiments including the analysis of melting 
...
experimental practice and reliable interpretation of qPCR results. 

R is one of the most used tools in bioinformatics and is known to be an early 
adopter of emerging technologies \citep{pabinger_2014}. Recently we published 
the \CRANpkg{dpcR} package at CRAN, which is the first open source software package 
based on R for the analysis of digital PCR (dPCR) experiments. The dPCR 
...
\BIOpkg{NormqPCR}. All the packages are either available form CRAN or 
...
\strong{S3}, \strong{S4} and \strong{R6}) vary considerable depending on the 
...
cycle threshold method (Ct) (\code{th.cyc}, \CRANpkg{chipPCR}). linear 
regression $R^{2}~=~0.9945$ ($P < 2.2^{-16}$) and Pearson's $r~=~0.9972605$ ($P 
< 2.2^{-16}$), \strong{(D)}, \strong{(E)}} 
  \label{figure:dilution_Cq}
\end{figure}

\subsection{Example two}

In this study we used the \CRANpkg{RDML} package to read the qPCR experiment. 
A good practice for 
...
package provides this information. Provided that the analysis starts with a 
clean R session it is possible to assign the required packages to an object 
only, as shown in our example below.
...
workflowz and to setup environment for specific needs in a broad range of 
technical settings (Figure~\ref{figure:options}). R is no monolithic 
integration. We claim that the modular structure of R packages allows user to 
perform flexible data analysis adjusted to their needs. R allows to access and 
...
611X. We would like to thank R community who created this environment for the 
analysis of qPCR data. Part of this work was funded by the Russian Ministry of 
Education and Science (project RFMEFI62114X0003) and with usage of equipment of 
