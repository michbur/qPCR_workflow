...
work hard to uncover pitfalls of qPCR experiments. Finnaly this leads to the 
...
and guidelines for a proper conduction of qPCR experiments as implemented in the 
...
were emerged several isothermal amplification technologies, such as helicase 
...
results \citep{huggett_BDQ_2014}. Yet, in case of closed source software usually the 
analysis happens in a black~box fashion tied to a specific 
...
However, at least open~source gives the possibility to track and eliminate  
errors by an individual entity. Often black~box systems have limitations in the data 
processing and force the user to process the data by suboptimal analysis 
algorithms \citep{ruijter_2013}. The visualization options are usually limited 
by the software and does not meet publication quality. Users who have no 
access to the commercial software are barred. Aside from closed source software, 
often analysis of data performed in spreadsheets. However, this data 
processing approach is not advisable for research purposes. Frequently spreadsheets  
...
Murrell_2012, gandrud_2013, hofmann_2013, Leeper_2014, liu_2014}. Also, 
...
of particular importance to the human -- machine interface \citep{Oh_2014}.
...
extension steps \citep{rodiger_nucleic_2014}. The corresponding signal is 
monitored continuously. Therefore, the $Cq$ value depends on the time instead of 
cycles. In this study we used the \code{th.cyc} function to determine the time 
required to reach a certain threshold level ($Cq_{t}$).
...
Next we used the \code{CPP} function to pre-process the raw data. Since the raw 
data showed a slight negative trend and an off-set of circa 0.4 RFU 
(Figure~\ref{figure:qIA}A). Similar to the other case studies we baselined and 
smoothed the data. In addition, outliers were automatically removed in the 
baseline region (Figure~\ref{figure:qIA}A~and~B).
...
We calculated a cycle threshold time    70.18   93.18 minutes, receptively.
