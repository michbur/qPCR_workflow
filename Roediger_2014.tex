...
distributed as the closed source software or as monolithic block with little freedom 
...
packages available. In the paper, we describe exemplary workflows for the analysis of 
dPCR, qIA or qPCR experiments including the analysis of melting curve data. Our 
analysis relies entirely on R packages available from public repositories. Additionally, we provide information related to standardized and reproducible 
research with R.
}
...
precise quantification of minute DNA traces is required. The applications include 
...
dependent amplification (HDA), as alternative to PCR. The isothermal amplification 
...
chambers) \citep{huggett_2013, milbury_2014, morley_2014}. The partitions with no 
...
calculated from the amplification curves and converted into the discrete events by 
...
results \citep{huggett_BDQ_2014}. The scientific misconduct and fraud have shaken 
...
In case of closed source software usually the analysis happens in a black~box 
...
complicated tasks. For the more elaborated critique see \citet{mccullough_2008, 
burns_2014}.
...
plugin-like architecture. The R is an independent, cross-platform instrument and 
provides a broad spectrum of calculation options. Particularly, the visualization of 
...
readily-available for further processing. This is of the particular importance to 
...
(v.~0.4-2) to our workspace. The \CRANpkg{chipPCR} package (v.~0.0.8-3) was used 
...
\strong{(B)} The \code{CPP} function from the \CRANpkg{chipPCR} was sued to 
...
\CRANpkg{archivist} package, which stores and recovers crucial data and 
preserves metadata of saved objects (not shown). All settings of R session can 
be easily saved and/or restored using \CRANpkg{settings} package.
...
\code{th.cyc} function to calculate the cycle threshold time for samples D1 and 
...
experiments with R. In addition to this, we briefly referenced to a vast collection of 
additional packages available from CRAN and Bioconductor. The packages may be 
considered as the building blocks (libraries) to create what users want and need. We 
...
integration. We claim that the modular structure of R packages allows the user to 
perform flexible data analysis, adjusted to their needs and to design frameworks 
for high-throughput analysis. Furthermore, R enables the user to access and reuse code for the creation 
...
advantage of this software is its transparency at any time for anybody. Thus, it is 
possible to track numerical errors.  A serve disadvantage of R is the lack of 
comprehensive GUIs for qPCR analysis. Others and we believe that a graphical user 
...
novices. This problem we try to solve with using easy accessible 
GUIs \citep{rodiger_rkward_2012}. However, the work on this additions has been 
recently started and therefore it is still in progress.
