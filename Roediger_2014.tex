...
In case of closed source software usually the analysis happens in a 
...
read-in data in standardized format. However, any format supported by R could be 
used. Further processing of amplification curve data was performed with the 
\CRANpkg{chipPCR} package and amplification curve data were analysed with the 
\CRANpkg{MBmca} package. Cq, quantification cycle; $T_{M}$, melting temperature. 
...
\citep{ritz_2008, spiess_2008} (v.~1.4.0) and the \CRANpkg{RDML} package (v.~0.4-2) to our 
workspace. The \CRANpkg{chipPCR} package (v.~0.0.8-3) was used 
...
open data exchange formats are used. In this study we used the \CRANpkg{RDML} 
...
required to reach a certain threshold signal level ($Cq_{t}$).
