...
experiments is one of R's pinnacles. R enable an 
...
statement, which checks if $T_{M}$ is within a tight temperature range. We used 
...
used. In this study we used the \CRANpkg{RDML} package for data read-in. The 
data were measured with a CFX96 system (Bio-Rad) and then exported as RDML v1.1 
format file. The example file 
