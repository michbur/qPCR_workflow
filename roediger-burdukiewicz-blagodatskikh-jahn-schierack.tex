...
\title{R as an Environment for the Reproducible Analysis of DNA Amplification Experiments}
...
Moreover, quantitative isothermal amplification (qIA) methods have become more 
prominent in life sciences and point-of-care-diagnostics. Additionally, the 
analysis of melting data is essential during many experiments. Several software 
packages have been developed for the analysis of such datasets. In most cases, 
the software is either distributed as closed source software or as monolithic 
block with little freedom to perform highly customized analysis procedures. We 
argue, among others, that R is an excellent foundation for reproducible and 
transparent data analysis in a highly customizable cross-platform environment. 
However, for novices it is often challenging to master R or learn capabilities 
of the vast number of packages available. In the paper, we describe exemplary 
workflows for the analysis of qPCR, qIA or dPCR experiments including the 
analysis of melting curve data. Our analysis relies entirely on R packages 
available from public repositories. Additionally, we provide information related 
to standardized and reproducible research.
}

\section{Introduction}

Quantitative Polymerase Chain Reaction (qPCR) is the method of choice when 
a precise quantification of minute DNA traces is required. Applications 
...
\citet{milbury_2014}. Recently, we have published the \CRANpkg{dpcR} package at 
CRAN, which is the first open source R software package for the analysis of dPCR 
experiments (see \CRANpkg{dpcR} package manual for details).
...
data pre-processing, analysis, post-processing, visualization \citep{Murrell_2012, Murrel_2015} and storage. As 
...
\BIOpkg{NormqPCR}. All packages are either available from CRAN or 
...
\CRANpkg{RNetCDF} package, has advantages over some other binary formats (e.g., 
...
graphics. In our scenario we used the \CRANpkg{RDML} package to read-in data in 
...
package and melting curve data were analysed with the \CRANpkg{MBmca} package. 
The \CRANpkg{dpcR} package can be embedded in the analysis of digital PCR 
...
\CRANpkg{qpcR} package \citep{ritz_2008} (v.~1.4.0) and the \CRANpkg{RDML} 
package (v.~0.8-3) to our workspace. The \CRANpkg{chipPCR} package 
...
the \CRANpkg{MBmca} package (0.0.3-4) for an analysis of the target specific 
...
experiments. In particular, we used the \CRANpkg{dpcR} package (v.~0.1.4.0) to 
...
a qPCR experiment. We used the \texttt{guescini1} dataset from the \CRANpkg{qpcR} 
...
\code{sessionInfo} from the \CRANpkg{utils} package provides this functionality. 
...
We previewed the amplification curve raw data using the \code{matplot} function 
...
all data were subjected to a minimum-maximum normalization using the \code{CPP} 
function from the \CRANpkg{chipPCR} package. In addition, all data were baselined and 
...
(38~--~62 RFU). \strong{(B)} The \code{CPP} function from the \CRANpkg{chipPCR} 
package was used to baseline the data, to smooth the data with Savitzky-Golay 
smoothing filter and to normalize the data between 0 and 1. The red horizontal 
line (\textcolor{red}\textemdash) indicates the fluorescence level (0.05) used 
...
from the \CRANpkg{chipPCR} package. In our case study we needed to rearrange the 
...
Finally, Cq values were plotted using the \code{layout} function 
...
call \code{BioRad\$AsDendrogram()}. Names are used according to the RDML 
...
y)$) in the \code{CPP} function.). The curvatures of 'D1\_Alm12\ldots' and 
...
uniformly spaced. Our \code{CPP} function gives a warning in such 
a case. In qIA the $Cq$ values are dependent on the time instead of cycles. In 
this study we used the \code{th.cyc} function from the \CRANpkg{chipPCR} package 
...
First, we had a look at the \texttt{C81} dataset with the \code{str} 
function.
...
in seconds according to the \CRANpkg{chipPCR} manual) and the signal hight ('\$ 
...
\code{CPP} functions are just a reminder for the user to take care during the pre-processing. 
...
baseline region (0~-~52 minutes). \strong{(B)} We used the \code{CPP} 
...
(corresponds to baseline region between 0~and~52 minutes). The \code{CPP} uses a 
...
analysis of simulated dPCR experiments with the \CRANpkg{dpcR} package. In the 
...
\citep{jahn_2013, jahn_2014} and the manual of the \CRANpkg{dpcR} package. 
...
analysis. According to the \CRANpkg{dpcR} manual, the samples A02, 
...
about the number of positive partitions for the  plasmid DNA marker \textit{styA}. 
...
\begin{example}
  ...
  # Our next line is used to limit the x-axis for plot in a meaningful range.
...
As proposed by \citet{corbisier_2015} the analysis with the non-corrected 
...
of the analysis of digital PCR experiments. The \CRANpkg{dpcR} package offers a 
flexible wrapper \code{test\_counts} around several methods of comparing results 
of several reactions. In the following, we used a test implemented in 
\CRANpkg{rateratio.test} package by \citet{Fay_2010}.
...
Output format of \code{test\_counts} is familiar to all R users:
...
\begin{figure}[htbp]
  ...
  \caption{Comparison of droplet digital PCR runs. The \code{plot} method of the 
\code{test\_counts} function from the \CRANpkg{dpcR} package was used. The 
...
Our \CRANpkg{dpcR} package can be used to analyse data beyond a level as provided by 
...
\strong{(A)} R is typically run from a desktop computer and operated by a 
...
\bibliography{roediger-burdukiewicz-blagodatskikh-jahn-schierack}
