...
qPCR \citep{huggett_qpcr_2015}. Numerous commercial and experimental monitoring platforms 
...
nucleic acids and can be seen as a next generation method \citep{huggett_qpcr_2015}. The dPCR technology 
...
the scientific community on several occasions \citep{bustin_2014}. In particular, 
...
plugin-like architecture. The R is an independent, cross-platform analysis platform and 
...
(v.~0.4-2) to our workspace. The \CRANpkg{chipPCR} package (v.~0.0.8-6) was used 
...
experiments. In particular, we used the \CRANpkg{dpcR} package (0.1.3.3) to estimate the 
...
\code{th.cyc} function (\CRANpkg{chipPCR}) to calculate the cycle threshold time for samples D1 and 
...
This study gave an introduction to the analysis of qPCR, qIA, MCA and dPCR 
