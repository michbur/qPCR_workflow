...
So far we were able to calculate the Cq values for all samples. Next we focused 
...
uniformly spaced in qIA. Our \code{CPP} function gives a warning in such 
...
\code{CPP} functions is just a reminder for to user to take care during the 
...
droplet volume leads to an underestimation of sample concentrations 
...
(Figure~\ref{figure:dpcR_bioamp}~column~2). The positive samples seem to differ 
from the negative control (Figure~\ref{figure:dpcR_bioamp}~G04). This is 
basically a comparison of proportions \citep{wang_2013}. Statistical 
capabilities of R simplifies next steps of the analysis of digital PCR 
experiments. The \CRANpkg{dpcR} package offers a flexible wrapper 
\code{test\_counts} around several methods of comparing results of several 
reactions. In the following example we use test implemented in 
...
\begin{figure}[htbp]
  ...
  \caption{\code{plot} method for the output of the \code{test\_counts} function 
from the \CRANpkg{dpcR} package used for comparison of droplet digital PCR runs. 
Different experiments were aggregated into colour coded groups based on their 
estimated mean number of copies per partition ($\lambda$). A02 was a single group 
\textcolor{green}{b}, B02, C02 and D02 formed group \textcolor{blue}{c} and the 
negative control formed group \textcolor{red}{a}.
}
  \label{figure:test_counts}
\end{figure}

The result of the analysis suggests that the replicate samples B02, C02 and D02 
do not differ significantly but A01 is different. The negative control G04 was 
totally different from the other samples (Figure~\ref{figure:test_counts}). 
\CRANpkg{dpcR} package can be used not only to analyse data on level provided by 
the commercial software. Access to the whole R environment allows performing 
more elaborated research in a highly customizable workflow. In Conclusion, our 
