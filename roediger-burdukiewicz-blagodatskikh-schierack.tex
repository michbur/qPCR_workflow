...
First we had a look at the \texttt{C81} dataset with the \code{str} function.
...
Statistical capabilities of R simplifies next steps of the analysis of digital PCR experiments. \CRANpkg{dpcR} package offers flexible wrapper \code{test\_counts}  around several methods of comparing esults of several reactions. In the following example we use test implemented in \CRANpkg{rateratio.test}.
...
\begin{figure}[htbp]
  ...
  from the \CRANpkg{dpcR} package used 
for comparision of droplet digital PCR runs. Different experiments were agreggated into color coded groups based on their estimeted mean number of copies per partition. 
}
  \label{figure:dpcR}
\end{figure}

\CRANpkg{dpcR} package can be used not only to analyse data on level provided by the commercial software. Access to the whole R environment allows performing more elaborated research in a highly customizable workflow. 
